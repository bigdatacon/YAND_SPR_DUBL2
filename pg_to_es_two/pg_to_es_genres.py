...
class PGtoESGenres(PGLoader, ESSaver):
    def __init__(self, batch_size: int = 100):
        self.state = State(JsonFileStorage())
        self.batch_size = batch_size
        self.schemes = Schemes()
        self.index_name = 'genres_test'

    """1 Блок функций для заливки в эластик изменений по персонам"""

    def sync_persons_changes(self, index_name: Optional[str] = 'genres_test'):
    ...
    def find_genres_id_after_update(self):
        ...
        genres_ids_where_person_changed = set(i.get('id') for i in res)
        return genres_ids_where_person_changed

    def find_film_change_where_genres_changed(self, genres_ids_where_person_changed):
        sql_2_film_change_where_person_changed = """SELECT fw.id, fw.updated_at \
                                                FROM content.film_workmovie fw \
                                                LEFT JOIN content.genre_film_work gfw ON gfw.film_work_id = fw.id WHERE gfw.genre_id IN ('{}') ORDER BY fw.updated_at""".format(
            "','".join(genres_ids_where_person_changed))

        res_2 = self.do_query(sql_2_film_change_where_person_changed)
        film_ids_where_genres_changed = set(i.get('id') for i in res_2)
        return film_ids_where_genres_changed

    """убрал для простоты пока created_at и updated_at и p.id, из схемы эластик """

    def find_all_film_data_where_genres_changed(self, film_ids_where_genres_changed):
        sql_3_all_film_data_where_genres_changed = """SELECT
                                                    fw.id as id,
                                                    fw.title,
                                                    fw.description,
                                                    fw.rating,
                                                ...
                                                """.format("','".join(film_ids_where_genres_changed))
        res_3 = self.do_query(sql_3_all_film_data_where_genres_changed)
        return res_3
...
if __name__ == '__main__':
    example = PGtoESGenres()
    index_name = 'genres_test'
    last_state = example._PGtoESGenres__get_last_updated(index_name)
    print(f'here last_state : {last_state}')

    # 1проверка     print(example.find_person_id_after_update())
    genres_ids_where_person_changed = example.find_genres_id_after_update()

    # 2 проверка find_film_change_where_person_changed
    film_ids_where_genres_changed = example.find_film_change_where_genres_changed(genres_ids_where_person_changed)

    # 3 проверка find_all_film_data_where_person_changed
    res_3 = example.find_all_film_data_where_genres_changed(film_ids_where_genres_changed)
    print(res_3)
